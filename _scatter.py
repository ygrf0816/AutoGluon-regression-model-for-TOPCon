...
def dependence_legacy(ind, shap_values=None, features=None, feature_names=None, display_features=None,
    ...
    if interaction_index is not None:
        ...
        # 创建颜色映射
        norm = mcolors.Normalize(vmin=min(cvals[xv_notnan]), vmax=max(cvals[xv_notnan]))
        color_list = [cmap(norm(val)) for val in cvals[xv_notnan]]
        im1 = pl.cm.ScalarMappable(norm=norm, cmap=cmap)#used for colorbar
        for x, y, c in zip(xv[xv_notnan], s[xv_notnan],color_list):
            # 绘制散点
            p = ax.scatter(x, y, s=dot_size, linewidth=0.05,color=c,cmap=cmap,norm=color_norm,
                           rasterized=len(xv) > 500,edgecolors='black', alpha=alpha)

            # # 计算阴影偏移量
            # angle = np.arctan2(y, x)
            # x_offset = 0.2 * np.cos(angle)
            # y_offset = 1 * np.sin(angle)
            # # x_offset = 0
            # # y_offset = 0
            
            # # 绘制阴影
            # ax.scatter(x - x_offset, y + y_offset, s=(1/15)*dot_size, c='white', alpha=0.4)
            ax.scatter(x-x*0.0009, y+y*0.0025, s=(1/15)*dot_size, c='white', alpha=0.4)#用于线性类型
            # ax.scatter(x-(1/10000)*x, y+(1/10000)*y, s=(1/15)*dot_size, c='white', alpha=0.4)#指数型类型
        ...
        #     norm=color_norm, rasterized=len(xv) > 500
        # )
        # angles = np.arctan2(s[xv_notnan], xv[xv_notnan])
        # x_offsets = 0.2 * np.cos(angles)
        # y_offsets = 1 * np.sin(angles)
        # ax.scatter(xv[xv_notnan]- x_offsets, s[xv_notnan]+ y_offsets, s=(1/15)*dot_size, 
        #    c='white',alpha = 0.4, zorder=2)#shade
        # p.set_array(cvals[xv_notnan])
    else:
        for x, y in zip(xv, s):
            # 绘制散点
            p = ax.scatter(x, y, s=dot_size, linewidth=0.05, color=color,
                           rasterized=len(xv) > 500, edgecolors='black', 
                           alpha=alpha)
            ...
            # 绘制阴影
            ax.scatter(x - x_offset, y + y_offset, s=(1/15)*dot_size, c='white', alpha=0.4)
        # p = ax.scatter(xv, s, s=dot_size, linewidth=0, color=color,edgecolors='black',
        #                alpha=alpha, rasterized=len(xv) > 500)
        # angles = np.arctan2(s, xv)
        # x_offsets = 0.2 * np.cos(angles)
        # y_offsets = 1 * np.sin(angles)
        # ax.scatter(xv- x_offsets, s+ y_offsets, s=(1/15)*dot_size, 
        #    c='white',alpha = 0.4, zorder=2)#shade
    if interaction_index != ind and interaction_index is not None:
        # draw the color bar
        if isinstance(cd[0], str):
            ...
            #cb = pl.colorbar(p, ticks=tick_positions, ax=ax, aspect=80)
            cb = pl.colorbar(im1, ticks=tick_positions, ax=ax, aspect=80)
            cb.set_ticklabels(cnames)
        else:
            cb = pl.colorbar(im1, ax=ax, aspect=80)
        if colorname!=None:
            cb.set_label(colorname, size=16)
        else:
            cb.set_label(feature_names[interaction_index], size=16)
...
#         cb.ax.set_aspect((bbox.height - 0.7) * 20)
    ...
    ax.xaxis.set_ticks_position('bottom')
    ax.yaxis.set_ticks_position('left')
    ax.spines['right'].set_visible(is_cover)
    ax.spines['top'].set_visible(is_cover)
    ax.tick_params(color=axis_color, labelcolor=axis_color, labelsize=11,which='both', direction='in', 
